...
def main():
    ...
    video = ks.load_tif("../data/1 aging_00000221.tif") # TODO: for performance gains: write the centered video data to file, but first check if it exists
    ...
    knee_name = "aging-3" 
    coords, metadata = ks.load_aging_knee_coords("../data/198_218 updated xy coordinates for knee-aging 250426.xlsx", knee_name)
    coords_ctrd = ks.translate_coords(translation_mxs, coords) # Processes *some* frames
    coords_ctrd = ks.smooth_coords(coords_ctrd, 5)

    ks.plot_coords(video_ctrd, coords_ctrd) # Validate smoothing

    # Get masks
    regions, masks = ks.get_three_segments(video_ctrd, coords_ctrd)  
    keys = ["l", "m", "r"]

    ks.display_regions(regions, keys) # Validate regions
    
    # Get intensity data
    raw_intensities = ks.measure_region_intensities(regions, masks, keys) # Returns a dict
    normalized_intensities = ks.measure_region_intensities(regions, masks, keys, normalized=True)

    # Plot intensities
    show_figs = True
    save_figs = False
    ks.plot_three_intensities(raw_intensities, metadata, show_figs, save_figs)
    ks.plot_three_intensities(normalized_intensities, metadata, show_figs, save_figs)
