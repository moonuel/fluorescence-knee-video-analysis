"""
Integrated script for converting TIF image stacks to centered grayscale NPY files.
Processes TIF stack input to temporary grayscale NPY chunks, applies centering in parallel,
...
def produce_frame_chunks(stack_path, queue, chunk_size, num_workers):
    stack = tif.imread(stack_path)
    if stack.ndim != 3:
        raise ValueError("TIF stack must be 3D (frames, height, width)")
    chunk_idx = 0

    for i in range(0, len(stack), chunk_size):
        chunk = stack[i:i+chunk_size]
        print(f"[Reader] Read chunk {chunk_idx} with {len(chunk)} frames")
        queue.put((chunk_idx, chunk))
        chunk_idx += 1

    print("[Reader] Finished reading TIF stack and pushed final chunk")
...
def grayscale_worker(queue, temp_dir, worker_id):
    ...
    while True:
        ...
        print(f"[Grayscale Worker {worker_id}] Processing chunk {chunk_idx} with {len(chunk)} frames")
        # Assuming TIF stack is already grayscale uint8
        gray_frames = chunk.astype(np.uint8)
        temp_path = os.path.join(temp_dir, f"chunk_{chunk_idx:05d}.npy")
        np.save(temp_path, gray_frames)
...
def main(stack_path, output_path, chunk_size=100, num_workers=cpu_count()):
    print("[Main] Starting integrated processing pipeline for TIF stack")

    # Load stack to get shape
    stack = tif.imread(stack_path)
    if stack.ndim != 3:
        raise ValueError("TIF stack must be 3D (frames, height, width)")
    ...
    try:
        # Grayscale conversion phase (assuming already grayscale)
        ...
        produce_frame_chunks(stack_path, queue, chunk_size, num_workers)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SyntaxError(f"{sys.argv[0]} expects two args: [file_in] [file_out]"
                          f"\n\tExample usage: {sys.argv[0]} stack.tif stack_centered.npy")

    file_in = sys.argv[1]
    file_out = sys.argv[2]

    if not os.path.isfile(file_in):
        raise FileNotFoundError("Input TIF stack not found.")
